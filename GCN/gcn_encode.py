...
class Conv_surface(nn.Module):
    ...
    def forward(self, neighbor_index: "(bs, vertice_num, neighbor_num)",
        ...
        neighbor_direction_norm = get_neighbor_direction_norm(
            vertices, neighbor_index)
        support_direction_norm = F.normalize(
            self.directions, dim=0)  #(3, s * k)
        ...
        theta = torch.max(
            theta, dim=2)[0]  # (bs, vertice_num, support_num, kernel_num)
...
class Conv_layer(nn.Module):
    ...
    def forward(self, neighbor_index: "(bs, vertice_num, neighbor_index)",
        ...
        feature_center = feature_out[:, :, :self.
                                     out_channel]  # (bs, vertice_num, out_channel)
        feature_support = feature_out[:, :, self.
                                      out_channel:]  #(bs, vertice_num, support_num * out_channel)
        ...
        activation_support = torch.max(
            activation_support,
            dim=2)[0]  # (bs, vertice_num, support_num, out_channel)
...
class Pool_layer(nn.Module):
    ...
    def forward(self, vertices: "(bs, vertice_num, 3)",
        ...
        pooled_feature = torch.max(
            neighbor_feature, dim=2)[0]  #(bs, vertice_num, channel_num)
        ...
        feature_map_pool = pooled_feature[:,
                                          sample_idx, :]  #(bs, pool_num, channel_num)
        return vertices_pool, feature_map_pool


class GCN3DFeat(nn.Module):
    ...
    def forward(self, vertices: "(bs, vertice_num, 3)"):
        ...
        feature_global = fm_4.max(1, keepdim=True)[0]
        # feature_global = fm_4#.max(1)[0]
        return feature_global


class GCN3DFeatDeep(nn.Module):
    def __init__(self, support_num: int, neighbor_num: int):
        super().__init__()
        self.neighbor_num = neighbor_num
        ...
        self.conv_3 = Conv_layer(128, 256, support_num=support_num)
        self.pool_2 = Pool_layer(pooling_rate=8, neighbor_num=8)
        self.conv_4 = Conv_layer(256, 256, support_num=support_num)
        self.conv_5 = Conv_layer(256, 256, support_num=support_num)
        self.pool_3 = Pool_layer(pooling_rate=8, neighbor_num=8)
        self.conv_6 = Conv_layer(256, 256, support_num=support_num)
